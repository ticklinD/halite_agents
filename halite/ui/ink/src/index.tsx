...
backend.on('stderr', (line: string) => {
  // The backend already writes full logs to ~/.halite/logs/halite_*.log.
  // Forwarding stderr here would interleave loguru lines with Ink's own
  // terminal writes, causing overlapping/duplicated "error" renders during
  // startup. Drop it — file logs are the source of truth for debugging.
})
